...
// Query 3: Determine the factories producing Product Gamma and their suppliers to assess disruption impact.
// params: {"product_id": "P3"}
MATCH (s:Supplier)-[:SHIPS_TO]->(f:Factory)-[:PRODUCES]->(p:Product) WHERE p.id = $product_id RETURN s.id AS supplier_id, f.id AS factory_id, s.country AS supplier_country LIMIT 50
// substituted
MATCH (s:Supplier)-[:SHIPS_TO]->(f:Factory)-[:PRODUCES]->(p:Product) WHERE p.id = "P3" RETURN s.id AS supplier_id, f.id AS factory_id, s.country AS supplier_country LIMIT 50
;
